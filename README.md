...
3. Если тестируете проект через Postman: 
   <li> а) Создайте образ проекта командой docker build -t <имя образа> .
   <li> б) Далее, применив команду docker-compose up, создайте и запустите контейнер
   <li> в) протестируйте приложение, указан в адресной строке **http://127.0.0.1:5000/api/meta(bold)** и применив запрос **GET**.
