...
3. Если тестируете проект через Postman: 
   <ul> а) Создайте образ проекта командой docker build -t <имя образа> .
   <ul> б) Далее, применив команду docker-compose up, создайте и запустите контейнер
   <ul> в) протестируйте приложение, указан в адресной строке **http://127.0.0.1:5000/api/meta (bold)** и применив запрос **GET**.
